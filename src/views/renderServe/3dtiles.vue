...
import ShadowQuery from "@/components/shadow-query/shadowQuery.vue";
...
const set3Dtitle3 = () => {
  ...
  const url = "http://192.168.16.42/3dtiles/scene.json";
  let tilesetJson = {
    url,
    modelMatrix: m,
    ...
    maximumScreenSpaceError: 16, // 数值加大，能让最终成像变模糊---用于驱动细节细化级别的最大屏幕空间误差(默认16)原128
  ...
  tileset.readyPromise
    .then(function (tileset) {
      window.$viewer.scene.primitives.add(tileset, 1);
    })
